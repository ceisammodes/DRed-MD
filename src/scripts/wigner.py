...
"""
SHARC Program Suite - Modified by Vincent Delmas - last edit: 22/05/2023
...
# Constants
DEBUG = False
CM_TO_HARTREE = 1. / 219474.6  # Conversion factor from cm-1 to Hartree: 4.556335252e-6
HARTREE_TO_EV = 27.211396132  # Conversion factor from Hartree to eV
U_TO_AMU = 1. / 5.4857990943e-4  # Conversion from g/mol to amu
ANG_TO_BOHR = 1. / 0.529177211  # Conversion from Angstrom to bohr: 1.889725989
PI = math.pi

# Threshold [cm^-1] to ignore rotational and translational low frequencies
LOW_FREQ = 10.0
...
class ATOM:
    def __init__(self, symb: str = "??", num: float = 0., coord: List = None, m=0., veloc: List = None):
        if veloc is None:
            veloc = [0., 0., 0.]
...
class STATE:
    def __init__(self, i: int = 0, e: float = 0., eref: float = 0., dip: List = None):
        if dip is None:
            dip = [0., 0., 0.]
...
class INITCOND:
    def __init__(self, atomlist: List = None, eref: float = 0., epot_harm: float = 0.):
        if atomlist is None:
            atomlist = []
    ...
    def init_from_file(self, f, eref, index):
        ...
        # Skip one line, where "Atoms" stands
        f.readline()
        atomlist = []
        while True:
            line = f.readline()
...
def import_from_molden(filename, scaling, flag, lvc=False):
    """
    This function imports atomic coordinates and normal modes from a MOLDEN file. Returns molecule and modes
    as the other function does.
    """
...
def determine_state(mode):
    """ This function determines the vibrational state of the mode for the system at a certain temperature. """
    ...
    if exponent > 800:
        ...
        print("The partition function is too close to zero due to very low temperature or very high frequency! "
              "It was set to %e" % (math.exp(-exponent / 2.) / (1. - math.exp(-exponent))))
    partition_function = math.exp(-exponent / 2.) / (1. - math.exp(-exponent))
...
def wigner(Q, P, mode, n=None):
    """
    This function calculates the Wigner distribution for a single one-dimensional harmonic oscillator.
    Q contains the dimensionless coordinate of the oscillator and P contains the corresponding momentum.
    n is the number of the vibrational state (default 0). The function returns a probability for this set of parameters.
    """
        ...
        if temperature == 0:
            ...
            if n > 500:
                if high_temp:
                    ...
                    print('The calculated excited vibrational state for this normal mode exceeds the limit of the '
                          'calculation.\nThe harmonic approximation is not valid for high vibrational states of '
                          'low-frequency normal modes. The vibrational state ', n,
                          ' was set to 150. If you want to discard these states instead (due to oversampling of '
                          'state nr 150), use the -T option.')
                    n = 500

    # Vibrational ground state
    if n == 0:
        return math.exp(-Q ** 2) * math.exp(-P ** 2), 0.
        # TODO: see eq (6) of Zobel 10.1039/C8CP03273D for a better way to sample the canonical ensemble
...
for the first 11 vibrational states."""
    ...
    for n in range(11):
        ...
        for Q in grid:
            for P in grid:
                W = wigner(Q, P, n)
                outstring += f"{Q:6.2f} {P:6.2f} {W: 12.8e}\n"
            outstring += "\n"
...
def determine_normal_modes_format(modes, molecule, nmodes, flag):
    """
    This function determines the input format of the normal modes by trying to transform them to mass-weighted
    coordinates and seeing which of the four methods was able to do so via checking if the normal modes are now
    orthogonal. The mass-weighted normal coordinates are then returned
    """
    ...
    normformat = ["gaussian-type (Gaussian, Turbomole, Q-Chem, AMS, Orca)", "cartesian (Molpro, Molcas)",
    ...
    if flag != 0:
        ...
        # create dotproduct matrices of the normal mode multiplication for all three transformations.
        ...
        possibleflags = []
        for i in range(4):
            if diagonalcheck[0][i] > nmodes - 1 and diagonalcheck[0][i] / nmodes - 1 < thresh \
                    and diagonalcheck[1][i] == 1:
        ...
        try:
            ...
            if 'MASS_LIST' in globals():
                print("Hint: When using non-default masses, the frequency calculation also has do be carried out "
                      "with the same non-default masses. Otherwise, wigner.py is unable to identify the "
                      "correct normal mode convention.")
            exit(1)
...
def sample_initial_condition(molecule, modes):
    """
    This function samples a single initial condition from the modes and atomic coordinates by the use of a
    Wigner distribution. The first atomic dictionary in the molecule list contains also additional information
    like kinetic energy and total harmonic energy of the sampled initial condition.
    Method is based on L. Sun, W. L. Hase J. Chem. Phys. 133, 044313 (2010) nonfixed energy, independent mode sampling.
    """
    ...
    for imode, mode in enumerate(modes):  # for each uncoupled harmonatomlist oscillator
        while True:
            ...
            # TODO: needs to be restructured: first obtain temperature, then draw random numbers,
            #  then compute wigner probability
...
def create_initial_conditions_list(amount, molecule, modes):
    """
    This function creates 'amount' initial conditions from the data given in 'molecule' and 'modes'.
    Output is returned as a list containing all initial condition objects.
    """
    ...
    if vibstate is not None:
        nvib = len(vibstate)
        if nvib < len(modes):
            print("ERROR: not enough quantum numbers given to -v! There should be at least %i quantum numbers." % (
                len(modes)))
            sys.exit(1)
        elif nvib > len(modes):
...
def lvc_input(molecule, modes):
    ...
    if not len(modes) == len(modes[0]['move'] * 3):
        ...
        if nr_missing != 6:
            print('There are %i normal modes missing that will be assumed to be null-vectors! Make sure this '
                  'is intended.' % nr_missing)
            time.sleep(2)
        empty_modes = [{'freq': 0.0, 'move': [[0, 0, 0] for x in range(len(modes[0]['move']))]} for x in
                       range(nr_missing)]
...
"""
    ...
    parser.add_option('-n', dest='n', type=int, nargs=1, default=3,
                      help="Number of geometries to be generated (integer, default=3)")
    parser.add_option('-m', dest='m', action='store_true', help="Enter non-default atom masses")
    parser.add_option('-s', dest='s', type=float, nargs=1, default=1.0,
                      help="Scaling factor for the energies (float, default=1.0)")
    ...
    parser.add_option('-T', dest='T', action='store_true',
                      help="Discard high vibrational states in the temperature sampling ")
    parser.add_option('-v', dest='v', type=str, nargs=1, default=None,
                      help="Give an explicit list of vibrational states for all modes (e.g., '0 0 1 0 0 ...'). "
                           "The string should contain one number for each considered mode after removing "
                           "low-frequency/zero-norm modes. If used, temperature is ignored. ")
    parser.add_option('-L', dest='L', type=float, nargs=1, default=10.0,
                      help="Discard frequencies below this value in cm-1 (float, default=10.)")
    parser.add_option('-o', dest='o', type=str, nargs=1, default='initconds',
                      help="Output filename (string, default=""initconds"")")
    ...
    parser.add_option('-l', dest='lvc', action='store_true',
                      help='Generate input for SHARC_LVC.py (V0.txt) rather than initconds')
    parser.add_option('-r', dest='r', type=int, nargs=1, default=16661,
                      help="Seed for the random number generator (integer, default=16661)")
    parser.add_option('-f', dest='f', type=int, nargs=1, default='0',
                      help="Define the type of read normal modes. 0 for automatic assignement, 1 for gaussian-type "
                           "normal modes (Gaussian, Turbomole, Q-Chem, AMS, Orca), 2 for cartesian normal modes "
                           "(Molcas, Molpro), 3 for Columbus-type (Columbus), or 4 for mass-weighted. "
                           "(integer, default=0)")
    parser.add_option('--keep_trans_rot', dest='KTR', action='store_true',
                      help="Keep translational and rotational components")
    parser.add_option('--use_eq_geom', dest='UEG', action='store_true',
                      help="For all samples, use the equilibrium geometry (only sample velocities)")
    parser.add_option('--use_zero_veloc', dest='UZV', action='store_true',
                      help="For all samples, set velocities to zero")
...
INPUT  file                  = {filename}
OUTPUT file                  = {outfile}
Number of geometries         = {amount}
Random number generator seed = {options.r}
Temperature                  = {options.t}
    ...
    for i, mode in enumerate(modes):
        string += f"{i + 1:4d} {mode['freq'] / CM_TO_HARTREE:12.4f}\n"
    print(string)
